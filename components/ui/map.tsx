...
> = {
  ...
  pointColor?: string;
  /** Radius for unclustered individual points (default: 5) */
  pointRadius?: number;
  /** Optional text label rendered over unclustered points */
  pointLabel?: string;
  /** Optional SVG markup for an icon rendered over unclustered points */
  pointIconSvg?: string;
  /** Icon size scale for the unclustered point icon (default: 0.6) */
...
>({
  ...
  const pointLabelLayerId = `unclustered-label-${id}`;
  const pointIconLayerId = `unclustered-icon-${id}`;
  const pointIconId = `point-icon-${id}`;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try {
        ...
        if (map.hasImage(pointIconId)) map.removeImage(pointIconId);
  ...
  useEffect(() => {
    ...
    if (map.getLayer(pointIconLayerId) && prev.pointIconSize !== pointIconSize) {
      map.setLayoutProperty(pointIconLayerId, "icon-size", pointIconSize);
    }
